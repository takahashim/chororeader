...
enum ReaderScripts {
    ...
    pre:hover > .tzr-code-actions { display: inline-flex; }
    .tzr-code-button {
    ...
    .tzr-code-button:hover { background: #fff; }
    #tzr-chapter-end {
        margin: 3em 0 1em; padding-top: 1.5em;
        border-top: 1px solid rgba(128,128,128,0.25);
        text-align: center;
    }
    .tzr-chapter-end-button {
    ...
    .tzr-chapter-end-button:hover { background: rgba(128,128,128,0.18); }
    """
    ...
    static func styleScript(css: String) -> String {
        ...
        (function () {
          ...
          window.tzrMake = function (tag) { return document.createElementNS(XHTML, tag); };
          window.tzrSetStyle = function (css) {
            var el = document.getElementById('tzr-style');
            if (!el) {
              el = window.tzrMake('style');
              el.id = 'tzr-style';
              (document.head || document.documentElement).appendChild(el);
            }
            el.textContent = css;
          };
          window.tzrSetStyle(\(quote(css)));
        })();
        """
    }
    ...
    (function () {
      if (window.__tzrReady) return;
      window.__tzrReady = true;

      function post(m) {
        try { window.webkit.messageHandlers.tzr.postMessage(m); } catch (e) {}
      }
      function scrollMax() {
      ...
      window.tzrSetChapterEnd = function (label) {
        var existing = document.getElementById('tzr-chapter-end');
        if (existing) { existing.remove(); }
        if (!label) return;
        var box = window.tzrMake('div');
        box.id = 'tzr-chapter-end';
        var button = window.tzrMake('button');
        button.setAttribute('class', 'tzr-chapter-end-button');
      ...
      function decorateCode() {
        ...
        for (var i = 0; i < pres.length; i++) {
          var pre = pres[i];
          if (pre.getAttribute('data-tzr')) continue;
          pre.setAttribute('data-tzr', '1');
          if (!pre.style.position) pre.style.position = 'relative';

          var bar = window.tzrMake('span');
          bar.setAttribute('class', 'tzr-code-actions');

          (function (pre, bar) {
            var copy = window.tzrMake('button');
            copy.setAttribute('class', 'tzr-code-button');
            copy.textContent = 'コピー';
            copy.addEventListener('click', function (e) {
              e.preventDefault(); e.stopPropagation();
              var clone = pre.cloneNode(true);
              var junk = clone.querySelectorAll('.tzr-code-actions');
            ...
            var wrap = window.tzrMake('button');
            wrap.setAttribute('class', 'tzr-code-button');
      ...
      function internalHref(a) {
        ...
        if (!href) return null;
        if (/^[a-z][a-z0-9+.-]*:/i.test(href) && href.indexOf('tzreader:') !== 0) return null;
        return href;
      }
      ...
      window.tzrApplyForeground = function (enabled) {
        var marked = document.getElementsByClassName('tzr-fg');
        while (marked.length) { marked[0].classList.remove('tzr-fg'); }
        ...
        while (stack.length) {
          ...
          if (tag === 'PRE' || tag === 'CODE') { el.classList.add('tzr-fg'); count++; continue; }
      ...
      window.tzrScrollToProgression = function (p) {
        window.scrollTo(0, scrollMax() * p);
        return true;
      };
      window.tzrScrollToFragment = function (id) {
      ...
      window.tzrScrollToText = function (needle) {
      ...
      window.tzrSelectedText = function () {
