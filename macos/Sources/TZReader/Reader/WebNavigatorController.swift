...
final class WebNavigatorController: NSObject, ObservableObject, WKNavigationDelegate,
    ...
    func copySelection() {
        webView.evaluateJavaScript("window.tzrSelectedText()") { value, _ in
    ...
    private func applyStyleToLivePage() {
        ...
        applyTheme()
        let css = settings.userCSS() + "\n" + ReaderScripts.chromeCSS
        webView.evaluateJavaScript("window.tzrSetStyle && window.tzrSetStyle(\(ReaderScripts.quote(css)))")
    ...
    private func applyForegroundMarking() {
        ...
        webView.evaluateJavaScript("window.tzrApplyForeground && window.tzrApplyForeground(\(enabled))")
    }
    ...
    private func restore(_ target: Locator) {
        if let fragment = target.fragment, !fragment.isEmpty {
            webView.evaluateJavaScript("window.tzrScrollToFragment(\(ReaderScripts.quote(fragment)))") { [weak self] ok, _ in
    ...
    private func restoreByTextOrProgression(_ target: Locator) {
        if let text = target.text, !text.isEmpty {
            webView.evaluateJavaScript("window.tzrScrollToText(\(ReaderScripts.quote(text)))") { [weak self] ok, _ in
                if (ok as? Bool) != true {
                    self?.webView.evaluateJavaScript("window.tzrScrollToProgression(\(target.progression))")
                }
            }
        } else if target.progression > 0 {
            webView.evaluateJavaScript("window.tzrScrollToProgression(\(target.progression))")
        }
    }
    ...
    private func showChapterEndAffordance() {
        guard let index = currentIndex, index + 1 < readingOrder.count else {
            webView.evaluateJavaScript("window.tzrSetChapterEnd && window.tzrSetChapterEnd(null)")
            return
        }
        let next = readingOrder[index + 1].href
        let title = document.publication?.title(forHref: next)
        let label = title.map { "次の章へ　\($0)" } ?? "次の章へ"
        webView.evaluateJavaScript("window.tzrSetChapterEnd && window.tzrSetChapterEnd(\(ReaderScripts.quote(label)))")
    }
