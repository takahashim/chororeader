...
final class FixedLayoutNavigatorController: NSObject, ObservableObject, WKNavigationDelegate,
    ...
    private func html(for indices: [Int]) -> String {
        ...
        let items = indices.map { index -> String in
            ...
            case let .document(href):
                let url = ResourceSchemeHandler.url(forHref: href)?.absoluteString ?? ""
                element = "<iframe class=\"choro-page choro-page-frame\" src=\"\(url)\"></iframe>"
            case nil:
                element = ""
            }
        ...
        .choro-slot { display: flex; align-items: center; justify-content: center; }
        .choro-page { display: block; \(fit) }
        .choro-page-frame { border: 0; width: 100vw; height: 100vh; }
        </style></head>
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        ...
        (function () {
          var el = document.getElementById('choro-page-\(currentPage)');
    ...
    (function () {
      document.addEventListener('keydown', function (e) {
        ...
        e.preventDefault();
        try {
          window.webkit.messageHandlers.choro.postMessage({
