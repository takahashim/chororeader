...
final class ThemeColorTests: XCTestCase {
    ...
    func testDarkThemeForcesColorOnlyThroughMarkedClass() {
        settings.theme = .dark
        let css = settings.userCSS()
        XCTAssertTrue(css.contains(".tzr-fg"), "暗いテーマで印付けを使っていない")
    ...
    func testElementsWithBackgroundAreExcluded() throws {
        ...
        let marked = evaluate(webView, "window.tzrApplyForeground(true)") as? Int
        ...
        func hasClass(_ id: String) -> Bool {
            (evaluate(webView, "document.getElementById('\(id)').classList.contains('tzr-fg')") as? Bool) ?? false
        }
