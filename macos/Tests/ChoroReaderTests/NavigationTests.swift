...
final class NavigationTests: XCTestCase {
    ...
    private func makeController(_ name: String) throws -> WebNavigatorController {
        ...
        XCTAssertTrue(wait { self.evaluate(controller, "window.__choroReady === true") as? Bool == true },
                      "注入スクリプトが動いていません")
        return controller
    }
    ...
    func testChapterEndAffordanceAppearsExceptOnLastChapter() throws {
        ...
        XCTAssertTrue(wait(timeout: 5) {
            (self.evaluate(controller, "!!document.getElementById('choro-chapter-end')") as? Bool) == true
        }, "章末の行き先が出ていない")

        controller.goNextChapter()
        XCTAssertTrue(wait(timeout: 5) { !controller.isLoading && controller.currentIndex == 1 })
        XCTAssertTrue(wait(timeout: 5) {
            (self.evaluate(controller, "!!document.getElementById('choro-chapter-end')") as? Bool) == false
        }, "最後の章にも行き先を出してしまっている")
    }
}
