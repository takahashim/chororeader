...
| サイドバー | 目次・ページ・検索・しおり | 「ページ」は PDF と固定レイアウトのみ。リフローではタブを出さない |
| 舞台 | 本文または紙面 | ここだけが差し替わる |
| 下辺 | 状態、前後（章／ページ）、位置 | 単位が章か紙面かで言い換える |

macOS 版の「関連」（意味の近い箇所）は、Windows 版にはまだ無い。
意味の索引そのものを持っていないので、タブも出さない。

**選び分けはタブに見せる。** `RadioButton` をそのまま置くと丸が並び、
「どれか 1 つ」という意味は伝わってもタブには見えない。
`ToggleButton` を土台にした型を当てて、押し込んだ釦の見え方にする
（サイドバーの 4 つ、書棚の表紙／一覧、引き方の語句／似た内容）。

**見え方は Fluent（`ThemeMode="System"`）に任せる。**
.NET 9 から WPF に入ったもので、角丸・余白・アクセント色・明暗の追従が
標準部品にまとめて効く。**まだ実験扱い**なので、将来の .NET で見え方や
読み込み方が変わりうる。XAML から設定するぶんには `WPF0001` は出ない
（コードから設定したときだけ）。

覆いに出すものは、**地の色と文字の色を対で置く**。地だけを指定すると、
明暗が切り替わったときに白地に白文字になりうる。

**字形は専用の書体で描く**（`Segoe Fluent Icons, Segoe MDL2 Assets`）。
`☰` や `‹ ›` を本文と同じ書体で置くと、大きさも太さも基線も揃わない。
符号位置は Windows 11 と 10 で共通なので、2 つ並べて書けばどちらでも出る。
**私用領域の符号なので、書体が無いと何も出ない。**意味は `ToolTip` に持たせる。

**一覧は 1 行の文字列で出さない。** 当たりは「どこか」と「本文」で重さが違い、
目次は階層を持つ。読書の窓の 4 面は同じ体裁を使い回し、
2 行目（`Detail`）の有無で見え方を変える。
**字下げは余白でやる**（空白文字だと字形の幅が一定でないので揃わない）。

サイドバーは掴んで幅を変えられる。長い目次では 300 では足りない。

**角丸と影は自分で付ける。** Fluent は部品には付けるが、こちらが置いた `Border`
は対象外である。表紙の枠と表示設定の覆いがそれに当たる。
影は要素の外へ描かれるので、余白を空けておかないと切られる。

**`Border` は子を角丸で切り取らない。** 表紙は `Image` を重ねるのではなく、
`ImageBrush` で地として塗る。重ねると角が四角いままはみ出す。

窓の地（Mica）は Fluent が DWM 経由で敷く。窓に不透明な地を置かないこと。
...
### 決めていないこと・実機で見てから決めること

**実機を見ないと決められないもの。** 見え方は macOS では確かめられない。

- **書棚の「一覧」（`DataGrid`）と下辺（`StatusBar`）が Fluent で浮いていないか。**
  Fluent の文書は対応する部品の一覧を載せておらず「まだ穴が多い」とだけ書いている。
  この 2 つは古い部品なので取り残されやすい。周りが丸くなった中でここだけ角張って
  見えるようなら、自前で組み直す（`ListBox` ＋ 体裁、`Border` ＋ `Grid`）。
  **浮いていなければ触らなくてよい**
- **道具帯の字形が出ているか。** 私用領域の符号なので、書体が無いと空白になる。
  そのときは文字（`☰` など）へ戻す
- **表示設定の覆いの影が、縁で切れていないか。** 影は要素の外へ描かれるので、
  余白（`Margin="10"`）が足りなければ広げる

**手触りとして残っているもの。**

- **目次を一度押しで開けない。** いまは `MouseDoubleClick` だけで、`Enter` でも開かない。
  サイドバーの目次は一度押しで飛ぶのが普通である
- **検索欄に手掛かりが無い。** 空の枠が置いてあるだけで、何を入れる所か分からない。
  WPF は素で持たないので、重ねる必要がある

**畳むかどうかを決めていないもの。**

- **Swift 側の ONNX 出口**（`macos/Sources/ChoroConvert/ONNX*.swift`）。
  C# 版（`windows/ChoroReader.Convert`）と**吐くものがバイト単位で同一**であることは
  確かめてあるので、いつでも畳める。残しておくと同じものを 2 つ保守することになるが、
  片方を直したときの照合の相手にもなる。
  畳むなら、確かめる手立ては凍結済みの 10 件だけになる
