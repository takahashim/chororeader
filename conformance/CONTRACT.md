...
`fixed` の各ページに載る `viewport` は、その章が `meta viewport` で名乗る寸法である
（`{"width": 1200, "height": 1700}`）。拡大の枠を先に決めるために要る。
名乗っていないページでは出さない。`content` の並びは、1 つでも `=` を欠いていたら全体を捨てる。
